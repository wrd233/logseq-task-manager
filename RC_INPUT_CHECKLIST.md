...
- [x] DeepSeek closure gold-set eval（40 cases）：false READY = 0，false NOT_READY = 0；readiness 35/40；attribution 23/24；2 例 model-format failure fail-safe 为 null（详见 `docs/experience/CLOSURE_SEMANTIC_EVALUATION.md`）
- [x] 10+ MiniProject / 10+ Project 多轮 Agent conversation replay（24 conversations / 69 turns，scripted DSH 剧本，0 errors；真实 DeepSeek 对话质量另待后续观测）
