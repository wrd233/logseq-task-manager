...
4. 同一 Graph 重装/升级仍需提供 `--graph-path` 与 `--graph-id`；省略 Graph identity 会以
   ...
   数据库并把它提升为 authority；
5. 只有显式迁移 authority 时才传 `--database`。安装、升级和首次启动都不会自动执行 V1
   迁移；
6. 在 Logseq 的插件页“手动载入插件”，选择该稳定解压目录中的
   `task-copilot-plugin/`；更新后点击“重载”。
