...
- 包内 Runbook 与 HEAD 字节一致，稳定 Plugin 注册、真实 reload、quit/owned shutdown、
  reopen、Service 重连和 database authority 保持全部 PASS；
- 新增正式状态 `0`、Runtime `0`、Recovery 分支 `0`、Skill/Prompt/Validator `0`、
  写入权威 `0`；
- 关闭交付漂移 Partial `1`，新增长期 Partial `0`，净变化 `-1`；
- 风险变化：文档/代码/截图漂移 `LOW-MEDIUM→LOW`，其他复杂度风险不变。

### r5 稳定安装态生命周期（`70a7fe7`，历史证据）

- 关闭发布包稳定 Plugin 注册/reload/quit/reopen 证据 Partial `1`，新增长期
  Partial `0`，净变化 `-1`；
- 新增正式状态 `0`、Runtime `0`、Recovery 分支 `0`、Skill/Prompt/Validator
  版本 `0`、写入权威 `0`；
- 删除临时解压目录的 Plugin 注册和三张不可持续截图，当时保留了稳定
  `tmp/releases/...-r5` 注册的生命周期证据；当前截图与产物已由 r6 取代；
