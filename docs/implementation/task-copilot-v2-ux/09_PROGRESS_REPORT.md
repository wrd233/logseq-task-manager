...
8. P2-G Rebind 与 Restore 正常往返已在当前构建完成真实 Desktop Gate；下一步注入一次
   Restore failure，验证原库保持、恢复点保留、用户层 Recovery 与 reload，并继续补齐
   Rebind Recovery/Undo 用户指引和 Migration 产品入口；不得加入第二恢复状态源。
