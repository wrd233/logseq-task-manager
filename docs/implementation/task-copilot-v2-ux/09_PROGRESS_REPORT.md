...
| P2 | IN_PROGRESS_P2_AB_DONE_P2C_ALL_SOURCES_DONE_P2D_LIGHT_CONDITION_MEDIUM_HEAVY_CORE_DONE_P2E_MAIN_CHAIN_DESKTOP_DONE_RECOVERY_GATE_OPEN_P2F_SHADOW_PROVIDER_REPEAT_PASS_P2G_REBIND_RESTORE_LIFECYCLE_DESKTOP_DONE | P2-A+B DONE；P2-C/P2-D/P2-E 核心链；P2-F shadow/provider；P2-G Rebind + Restore frontstage lifecycle PASS | P2-C/P2-D/P2-E 正常主链有 Desktop；P2-F 无 UI；P2-G Rebind 与 Restore lifecycle current build DONE | P2-D/P2-E remaining OPEN；P2-F real-context/feedback/frontstage OPEN；P2-G Recovery/Undo guidance + Restore state-delta/failure + Migration OPEN |
...
8. P2-G Rebind 与 Restore 产品入口生命周期已在当前构建完成真实 Desktop Gate；下一步
   在专用测试库制造可辨认正式状态差异，完成旧快照逐字段读回、恢复点反向 Restore 与
   failure 注入，并继续补齐 Recovery/Undo 用户指引与 Migration 产品入口；不得加入第二
   恢复状态源。
