...
| Project Page Head / Context Recovery | P1-G Context Recovery 代表链 DONE；File Graph Page Head 有界隐藏；Project 原正文失联时安全零写入并使用用户语言指向系统状态重新连接，不暴露 Anchor/对象/运行时 | CURRENT：`p1-g-07`～`13`；失联失败态 `ui-project-missing-source-user-language-current-971c6db.jpg` | P1-F DB Graph Page Head 宿主 Gate；1.3.0 持续质量样本不阻断当前 Slice；Project 重入正常成功态仍需最新构建取证 |
