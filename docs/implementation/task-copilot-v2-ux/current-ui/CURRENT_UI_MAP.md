...
| Recovery / Rebind / Restore / Migration | Rebind 受控选择主链已完成；纠错指引自动 Gate 不复活 missing/conflict 旧 Anchor；Restore 正常往返已完成；新指引 Desktop、Restore 失败链与 Migration 仍未完成 | CURRENT：Rebind `p2-g-07`～`12`；Restore `p2-g-13`～`19` | Rebind 新成功态；Restore failure/Recovery；Migration 用户向导 |
