...
| Recovery / Rebind / Restore / Migration | Rebind 受控选择主链已完成；Restore 产品入口的目录、二次校验、确认、恢复点、Service 自停/同 Graph 重连与 reload 生命周期已完成；Recovery/Undo 引导、Restore 状态差异/失败链与 Migration 仍未完成 | CURRENT：Rebind `p2-g-07`～`12`；Restore `p2-g-13`～`17` | Rebind 专用恢复/Undo；Restore 状态差异往返/失败注入；Migration 用户向导 |
