...
| Project Closure | 首屏先显示待判断数量、未正式应用、退出安全和唯一主动作；逐目标原始依据与完整依据默认折叠。Provider error 保留用户判断并只给一个重试动作；stale 丢弃旧草稿并改为重新检查当前条件；真实 Provider Review 从结构化结果生成一句结论，长模型说明继续折叠。正式 Commit/reload/inverse Undo 与 receipt-backed 中断续跑链保持不变 | CURRENT UI：`ui-compression-05`、`06`、`08`（`f4acf77`）、Provider error（`7727770`）、Review/stale/reload（`662246a`）；正式链：`p2-e-01`～`18` | 真正不能安全续跑的 `RECOVERY_REQUIRED` 代表链 |
