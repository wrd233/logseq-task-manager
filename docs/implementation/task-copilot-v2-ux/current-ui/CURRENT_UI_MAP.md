...
| Project Closure | 确定性证据、用户判断、真实 Provider、HIGH Review、Commit、reload、专用 inverse Undo 主链已完成 | CURRENT：预览 `p2-e-01`～`04`；正式/Undo `p2-e-05`～`12` | Provider error/stale；Commit failure→RECOVERY_REQUIRED→resume→reload |
| Recovery / Rebind / Restore / Migration | Rebind 受控选择主链已完成；纠错指引自动 Gate 不复活 missing/conflict 旧 Anchor；Restore 正常往返已完成；Migration 已完成 ledger、session-only scan、逐项 Review、恢复基线/Import/Verify/Undo、HIGH Activation 与完成后只读交接 | CURRENT：Rebind `p2-g-07`～`12`；Restore `p2-g-13`～`19`；Migration scan `p2-g-20`～`25`、Review/Preview `p2-g-26`～`29`、Import/Verify/Undo `p2-g-30`～`37`、Activation `p2-g-38`～`42`、只读归档 `p2-g-43` | Rebind 新成功态；Restore failure/Recovery；Migration 失败/中断恢复、视觉 Gate |
