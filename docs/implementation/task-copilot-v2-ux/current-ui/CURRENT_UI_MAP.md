...
|---|---|---|---|
| 工具栏 / 现在 / 待我确认 / 项目 / 更多 | “现在”对象筛选、分组和卡片类型统一为中文；通用可推进卡片首屏只显示一个结论，完整事实折叠；卡片保持单一主动作；待审阅当前问题与历史记录分离；空态使用真实 Provider 能力而非 legacy Agent flag；候选区普通路径去除 Provider/Proposal/SQLite/UUID/type enum，首屏一个主操作 + 一个暂缓动作，其余处置折叠；已处置内容在 reload/recompute 后不再冒充新增 | CURRENT Now：`ui-now-chinese-single-conclusion-current-*`（`f1d0e1f`，1000/724 px）；Day 8 CURRENT：`day-08-candidate-card-current-light-6135820.jpg`、`day-08-disposition-rescan-current-light-318baab.jpg`；其他：`ui-compression-02`～`06`、`08` + `review-empty-current-dark-cd59228.png` | Now Light 宿主代表图；Attention helpful/noise 仍需独立 Gate；集中 P0/P1 宿主 Gate |
...
| Project Closure | 首屏先显示待判断数量、未正式应用、退出安全和唯一主动作；逐目标原始依据与完整依据默认折叠。Provider error 保留用户判断并只给一个重试动作；stale 丢弃旧草稿并改为重新检查当前条件；真实 Provider Review 从结构化结果生成一句结论。最新连续使用链明确“审阅方案尚未应用”，应用/reload/Undo 与最近修改都使用业务结论，Commit/Lifecycle/SQLite/Service/Audit 折叠 | CURRENT UI：`ui-compression-05`、`06`、`08`（`f4acf77`）、Provider error（`7727770`）、stale/reload（`662246a`）、Day 9 Review/应用/Undo/健康（`aad478c` / `f18cc72` / `7fe762d`）；正式链：`p2-e-01`～`18` | 真正 `RECOVERY_REQUIRED` 仍需受控故障证据；当前合同只恢复一致性，不前向 resume |
