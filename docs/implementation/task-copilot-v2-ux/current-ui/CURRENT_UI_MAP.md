...
| Project Closure | 首屏先显示待判断数量、未正式应用、退出安全和唯一主动作；逐目标原始依据与完整依据默认折叠。正式 Provider/Review/Commit/reload/inverse Undo 与 receipt-backed 中断续跑链保持不变 | CURRENT UI：`ui-compression-05`、`06`、`08`（`f4acf77`）；正式链：`p2-e-01`～`18` | Provider error/stale；真正不能安全续跑的 `RECOVERY_REQUIRED` 代表链 |
