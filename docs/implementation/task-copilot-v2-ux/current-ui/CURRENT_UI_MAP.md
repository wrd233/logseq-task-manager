...
|---|---|---|---|
| 工具栏 / 现在 / 待我确认 / 项目 / 更多 | “现在”对象筛选、分组和卡片类型统一为中文；通用可推进卡片首屏只显示一个结论，完整事实折叠；卡片保持单一主动作；待审阅当前问题与历史记录分离；候选处置在 reload/recompute 后不再冒充新增。十日回顾显示 Review 无积压、Project/More 克制，但 Now 为 Focus 1 + Next 10；现有 Dynamic Now Shadow 会遗漏刚恢复事项，尚不可替换 | CURRENT Now：Day 10 `day-10-now-*plugin-7fe762d-docs-f738f59.jpg`；Review/Project/More 同批 Day 10；Day 8 disposition 继续有效；既有窄栏 `ui-now-chinese-single-conclusion-current-narrow-f1d0e1f.png` | 在既有 Now 投影中验证连续性保护与普通 Next 上限；Attention helpful/noise 仍需独立 Gate |
