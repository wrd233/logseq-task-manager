...
| Block / Page 现场入口 | main Page 与普通 Block 可用；同一 UUID 移动后精确返回新位置，来源删除后安全关闭且不猜测；真实 Provider abstain 使用“暂时不需要整理”的用户语言；right-sidebar、Query 与 Block reference 在 Logseq 0.10.15 File Graph 均无可靠 Plugin identity，安全隐藏并要求先打开来源 | CURRENT：`p0-k-01`～`09` | 成功/失败/Undo 返回；中文 IME；未来 DB Graph 宿主差异 |
