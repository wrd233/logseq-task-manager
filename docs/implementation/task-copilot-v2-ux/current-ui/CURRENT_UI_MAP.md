...
| Recovery / Rebind / Restore / Migration | Rebind 受控选择主链已完成；Restore 的目录、二次校验、确认、可辨认状态差异、恢复点反向 Restore、Service 自停/同 Graph 重连与 reload 正常往返已完成；Recovery/Undo 引导、Restore 失败链与 Migration 仍未完成 | CURRENT：Rebind `p2-g-07`～`12`；Restore `p2-g-13`～`19` | Rebind 专用恢复/Undo；Restore failure/Recovery；Migration 用户向导 |
