...
| Recovery / Rebind / Restore / Migration | Rebind 受控选择主链已完成；纠错指引自动 Gate 不复活 missing/conflict 旧 Anchor；Restore 正常往返已完成；Migration ledger 已自动完成用户阶段翻译但尚无当前截图 | CURRENT：Rebind `p2-g-07`～`12`；Restore `p2-g-13`～`19` | Rebind 新成功态；Restore failure/Recovery；Migration 材料选择、逐项 Review、正式状态机与 Desktop |
