...
> CURRENT 取代说明（2026-07-31，exact build `8928861`）：本文后续各节是按时间累积的
...
## Release r8 Node guard → 稳定安装 → Task → quit/reopen

状态：`DONE_CURRENT_PACKAGE_DESKTOP_LIFECYCLE`

1. 用户将 r8 解压到稳定、用户可控目录并使用 Node `>=20.19 <21`；
2. Node 21+ 或过旧版本在配置、LaunchAgent 与 authority 变化前安全停止；
3. Installer 必须收到显式 Graph identity；无 identity 安全停止，不猜 database；
4. 不传 `--database` 的同 Graph 重装保持既有 graph key/database authority；
5. Plugin 从稳定 r8 目录注册；“现在”的 Task 依据 Focus/Anchor 只给一个主操作；
6. 完整退出 Logseq 后 owned Service 与 descriptor 停止，Launcher 保持；
7. 重开 Logseq 后 Service 自动重连，Plugin 仍来自同一稳定目录；
8. CLI `objects 14`、Doctor PASS，当前 Provider smoke 零正式写入。

当前结果：`task-copilot-v2-0.1.0-8928861-r8.zip` 的 `unzip -t`、Runbook/源码
一致性、Node runtime guard、安装 payload hash、Skill hash、authority、quit/owned
shutdown 和 reopen 均 PASS。r5/r6 的历史证据保留，但产物为
