...
| P2-2 MiniProject 原位重构 | Application `mini-project-restructure-proposal.ts`；Domain HIGH `CREATE_BLOCK`/`MOVE_BLOCK` 合同；ADR-0007；正式执行尚未接线 | Graph patch ledger、SemanticCommit、Undo | DOMAIN_EXTENSION | 原始事实丢失、部分 Commit | PARTIAL_PROPOSAL_CONTRACT_AUTOMATED：Preview→一个 HIGH Proposal，原材料 no-rewrite/no-delete，创建/移动含 UUID/hash/双向位置；通用单 Block Commit 继续 fail closed；待专用多步骤 ledger、逆序补偿/Undo 与 Desktop identity Gate |
