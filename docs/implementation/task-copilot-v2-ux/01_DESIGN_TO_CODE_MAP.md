...
| P2-1 MiniProject Grill Me | Application `grill-session.ts`；Service `mini-project-grill.ts`/`llm-grill-turn.ts`；`mini-project-modeling@1.0.0` | Context Package、Graph read bridge、Provider、Proposal | LLM_SKILL | 固定问卷；超范围读取；自动写入 | PARTIAL_LIVE_PROVIDER：精确子树/answer→focus/stale/零写入 route 自动 PASS；真实 DeepSeek 两轮 Provider+Validator PASS；当前 Desktop bridge 未连接，Plugin UI、完整 live route、预览/Proposal/Desktop 仍待实现 |
