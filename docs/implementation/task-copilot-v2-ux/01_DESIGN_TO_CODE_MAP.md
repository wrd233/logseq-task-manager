...
| P2-3 Project 创建 Grill Me | 当前 Project 原子创建直接表单；Application Grill 已新增无 Object authority 的 `PROJECT_CREATION` subject；Service `project-creation-grill.ts` 与 `project-creation-modeling@1.1.0` 已实现三来源自适应 focus 和独立 Page/Object 关系维度；三种 Grill route 与 Page/Mini stale/zero-write 自动 Gate PASS；Project Creation Preview Validator/Provider generator、公开 Client、authenticated Preview route 与 session-only handle 已自动接通 | Project prepare/finalize、schema v12、GrillPreviewSessionStore | LLM_SKILL | 绕过页面原子性；变成固定大问卷；创建前伪造 Object ID；client 上传或跨重启复用 Preview | IN_PROGRESS_PREVIEW_SERVICE_AUTOMATED：Blank real `deepseek-v4-flash` 产出 machine focus=outcome 且 object count=0；Preview route 自动通过 Blank/Page success、not-ready no-provider、stale discard 和 formal zero-write；Preview→HIGH Review/正式创建/Recovery/Undo/Desktop 仍 OPEN |
