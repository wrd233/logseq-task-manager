...
| P1-4 状态翻译层 | Application `status-narration.ts` + Plugin `status-narration-runtime.ts`；System、Proposal Review、Recent Changes 已消费统一契约，Now Work/Object 仍用既有投影 | Condition/due/review/blocker/Project current interface/Proposal/Commit/Anchor/System facts | UI_ORCHESTRATION | LLM 杜撰下一动作；字段式输出；另建恢复入口 | PARTIAL_UI_AUTOMATED：事实/推断/未知与 next-action eligibility 已锁定；内部状态退到折叠详情，Commit/Undo 仍由既有 Handler 判定；待 Object/Anchor/Now consumer、Desktop 与 LLM draft protocol |
