...
| P1-9 LLM 状态叙述/当前接口 | Application `unified-ux-output.ts`、Local Service `llm-ux-output.ts` + `project-context-recovery.ts`、`recover-context@1.0.0`；尚未接 Plugin UI | Provider、Schema、Validator、Proposal Review、Context Package Skill catalog | LLM_SKILL | 模型覆盖正式事实或写入 | PARTIAL_SERVICE_AUTOMATED：模型只引用机器 fact/action/evidence ID；Project 路由只接收 object/version，服务端构造 SQLite/关系/Focus/Anchor/Commit facts 与只读动作，生成前后重验版本且零持久化；待真实 Provider/Plugin consumer |
| P1-10 交互日志与版本 | Application `interaction-evidence.ts` session buffer；P1-G Provider provenance；Plugin `structured-logger.ts`/Runtime Diagnostics；Local Service `process-output.ts` | bounded strict allowlist、skill/prompt/model version | DERIVED_DATA | 保存私人正文、对象身份、Prompt、模型原始输出、异常正文/stack/cause、路径或 Key | PARTIAL_EVIDENCE_AUTOMATED：P1-G 结构事件、Plugin diagnostics/export 与 Service daemon output 均排除自由文本异常/路径；CLI 是主动前台反馈，live/golden 是默认关闭研究 Gate；用户处置、跨会话留存和噪声 dashboard 仍开放 |
