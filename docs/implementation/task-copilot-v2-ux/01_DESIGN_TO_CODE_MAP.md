...
| P2-3 Project 创建 Grill Me | 当前 Project 原子创建直接表单；Application Grill 已新增无 Object authority 的 `PROJECT_CREATION` subject；Service `project-creation-grill.ts` 与 `project-creation-modeling@1.1.0` 已实现三来源自适应 focus 和独立 Page/Object 关系维度；三种 authenticated route 与 Page/Mini stale/zero-write 自动 Gate PASS；独立 Project Creation Preview Validator/Provider generator 自动 PASS | Project prepare/finalize、schema v12 | LLM_SKILL | 绕过页面原子性；变成固定大问卷；创建前伪造 Object ID | IN_PROGRESS_PREVIEW_FOUNDATION：Blank real `deepseek-v4-flash` 产出 machine focus=outcome 且 object count=0；两次非法 shape 安全拒绝后通过强化 outputContract 恢复；Preview route、Review/正式创建、Desktop 仍 OPEN |
