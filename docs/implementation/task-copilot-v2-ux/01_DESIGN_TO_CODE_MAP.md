...
| P1-5 “现在”动态编排 | Application `dynamic-now-shadow.ts` 已建 SHADOW 三段；正式 UI 仍是 Focus/Next/Waiting Review | `v2-now-work.ts`、Focus、Condition/due/blocker、status narration | UI_ORCHESTRATION | 退化为所有 OPEN 列表 | PARTIAL_AUTOMATED_SHADOW：三段 inclusion/exclusion、容量、Focus ownership PASS；建议区为空，待 runtime 对照与 UI Gate |
