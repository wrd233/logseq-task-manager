...
| P1-1 注意力信号数据层 | V1 `AttentionSignal` 仅历史；V2 无当前 Signal | due/review/Proposal/Commit/Anchor/Doctor facts | DERIVED_DATA | 形成第二套 Inbox 或第二权威 | 先建立纯派生 schema/port，禁止修改正式对象；明确 invalidation/cooldown/provenance |
...
| P1-3 影子模式 | 尚无 | structured logger、规则/Prompt 版本 | DERIVED_DATA | 未展示数据却无限积累私人正文 | 只存来源事实引用/摘要与决策，不存完整正文；提供抽查和清理 |
