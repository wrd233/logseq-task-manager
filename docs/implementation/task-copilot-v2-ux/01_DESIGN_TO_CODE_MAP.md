...
| P1-4 状态翻译层 | Application `status-narration.ts` 已覆盖 Object/Proposal/Commit/Anchor/System 确定性契约；现有 UI 仍用 Now Work `reason` 与零散 message | Condition/due/review/blocker/Project current interface/Proposal/Commit/Anchor/System facts | UI_ORCHESTRATION | LLM 杜撰下一动作；字段式输出；另建恢复入口 | PARTIAL_AUTOMATED：事实/推断/未知与 next-action eligibility 已锁定；恢复/修复只给既有安全路由，待接 ViewModel 与 LLM draft protocol |
...
| P1-7 Project 顶部重入条 | Application `reentry-projection.ts` 纯投影；现有展开式 Project workspace 尚未替换 | schema v12、Condition、Focus、Ownership、Anchor、Commit | UI_ORCHESTRATION | 与现有 Project workspace 产生不同事实；完整对象树压过停留点 | PARTIAL_AUTOMATED：Recovery 优先、最多三个 Focus 直属进入点、Association 仅背景、上下文不足显式承认；待 Plugin consumer/Page slot |
