...
| P2-1 MiniProject Grill Me | Application `grill-session.ts` session-only turn contract；三问 Closure Agent；`design-project` Skill | Context Package、Provider、Proposal | LLM_SKILL | 固定问卷；超范围读取；自动写入 | PARTIAL_CONTRACT_AUTOMATED：机器从 evidence-specific uncertainties 选择 focus/readiness，模型草稿 fail closed 且无正式写权限；Service/Provider、多轮 authority 更新、预览/Proposal/Desktop 仍待实现 |
