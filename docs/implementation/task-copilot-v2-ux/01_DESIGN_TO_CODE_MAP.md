...
| P2-1 MiniProject Grill Me | Application `grill-session.ts`/`grill-preview.ts`；Service `mini-project-grill.ts`/`llm-grill-turn.ts`/`llm-grill-preview.ts`；Plugin `mini-project-grill-controller.ts`/session UI；`mini-project-modeling@1.1.0` | Context Package、Graph read bridge、Provider、Proposal | LLM_SKILL | 固定问卷；超范围读取；自动写入 | PARTIAL_PREVIEW_UI_AUTOMATED：精确子树/answer→focus/stale/零写入 route 自动 PASS；最终阅读预览强制原材料 exact-once、root 保留、零删除、越界 evidence fail closed；真实 DeepSeek turn 与 preview Validator PASS；Plugin 多轮/预览 UI、error 保留、stale/cleanup、Block origin 自动 PASS；当前 Desktop bridge 未连接，完整 live route、Proposal/Desktop 仍待实现 |
| P2-2 MiniProject 原位重构 | Application `mini-project-restructure-proposal.ts`；Service session handle + server-owned Proposal route；Plugin Preview→Review；Domain HIGH `CREATE_BLOCK`/`MOVE_BLOCK`；ADR-0007 | Graph patch ledger、SemanticCommit、Undo | DOMAIN_EXTENSION | 原始事实丢失、部分 Commit | PARTIAL_REVIEW_UI_AUTOMATED：Preview→一个 HIGH Proposal，原材料 no-rewrite/no-delete，创建/移动含 UUID/hash/双向 scoped 位置；handle 有 TTL/容量/重启边界，Service 重读来源且 client 不能上传 preview；通用单 Block Commit 继续 fail closed；待专用多步骤 ledger、逆序补偿/Undo 与 Desktop identity Gate |
