...
| P1-10 交互日志与版本 | Application `interaction-evidence.ts` session buffer；P1-G Provider provenance；通用 `structured-logger.ts` 待隐私审计 | bounded strict allowlist、skill/prompt/model version | DERIVED_DATA | 保存私人正文、对象身份、Prompt、模型原始输出或 Key | PARTIAL_SESSION_AUTOMATED：P1-G 成功/Validator 拒绝/Provider 失败只记录场景、结构计数、版本、固定错误码和时长；证据 sink 故障隔离；用户处置、跨会话留存、噪声 dashboard 与通用日志审计仍开放 |
