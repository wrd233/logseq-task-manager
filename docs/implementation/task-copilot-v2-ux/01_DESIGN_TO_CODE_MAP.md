...
| P1-4 状态翻译层 | Application `status-narration.ts` + Plugin `status-narration-runtime.ts`；System、Proposal Review、Recent Changes、Now Work 已消费统一契约 | Condition/due/review/blocker/Project current interface/Proposal/Commit/Anchor/System facts | UI_ORCHESTRATION | LLM 杜撰下一动作；字段式输出；另建恢复入口 | PARTIAL_UI_AUTOMATED：事实/推断/未知与 next-action eligibility 已锁定；Now 使用版本匹配 Object 并只复用 Condition Handler；Commit/Undo 仍由既有 Handler 判定；待 Anchor repair consumer、Desktop 与 LLM draft protocol |
...
| P1-7 Project 顶部重入条 | Application `reentry-projection.ts` + Plugin `reentry-runtime.ts` 已替换生产 Project workspace 的展开式卡片 | schema v12、Condition、Focus、Ownership、Anchor、Commit | UI_ORCHESTRATION | 与现有 Project workspace 产生不同事实；完整对象树压过停留点 | PARTIAL_UI_AUTOMATED：Recovery 优先、最多三个 Focus 直属进入点、Association 仅背景、上下文不足显式承认；待 Page slot/Desktop |
