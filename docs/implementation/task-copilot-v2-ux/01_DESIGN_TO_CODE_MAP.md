...
| P2-7 Recovery/Rebind/Restore/Migration 向导 | Plugin Diagnostics/Audit/Migration + `v2-anchor-rebind.ts`；CLI/独立 workspace | 既有安全链全部复用 | SERVICE_PRODUCTIZATION | 新向导形成第二恢复器；路径/UUID 暴露 | PARTIAL_REBIND_FRONTSTAGE_AUTOMATED：Rebind 阅读预览/成功反馈与 DOM token 不展示 UUID/Anchor/hash，既有确认/重验/Service command/replaced 历史不变；Desktop 与 Recovery/Restore/Migration 产品入口 OPEN |
