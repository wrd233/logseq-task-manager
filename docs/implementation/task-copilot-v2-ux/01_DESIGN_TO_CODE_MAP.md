...
| P2-6 跨对象观察 | Application `cross-object-shadow.ts` + Attention detector；Plugin session shadow runtime | bounded versioned subjects/scope/evidence/provenance；既有 Attention invalidation/cooldown | LLM_SKILL | 弱候选泛滥、正文泄露、自动 Ownership/Focus | PARTIAL_SHADOW_CONTRACT_AUTOMATED：五类 fixed draft、2–8 subject、2–16 evidence、每轮 8 条，只进 `LLM_CROSS_OBJECT/INFERENCE/SHADOW/NONE` 与 count-only telemetry；无 Provider/UI/persistence/Proposal/写入，待真实 Provider 质量 Gate 后再形成 Skill candidate 与确认链 |
