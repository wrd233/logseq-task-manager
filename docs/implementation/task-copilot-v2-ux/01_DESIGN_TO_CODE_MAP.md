...
| P2-2 MiniProject 原位重构 | Application Proposal + dedicated Commit planner；Service session handle/Proposal/prepare/step verify/recovery；Plugin Preview→Review；Domain HIGH `CREATE_BLOCK`/`MOVE_BLOCK`；ADR-0007 | Graph patch ledger、SemanticCommit、Undo | DOMAIN_EXTENSION | 原始事实丢失、部分 Commit | PARTIAL_LEDGER_AUTOMATED：Preview→一个 HIGH Proposal，原材料 no-rewrite/no-delete；完整来源/最终结构指纹、执行前与补偿位置分离；每操作 GRAPH_WRITE step、顺序 verify、最终指纹、逆序 compensation、FAILED/replay 自动 PASS。通用单 Block Commit 继续 fail closed；待 Plugin executor、完成态 Undo 与 Desktop insert/move/remove identity Gate |
