...
| P2-2 MiniProject 原位重构 | Application Proposal + dedicated Commit planner；Service session handle/Proposal/prepare/step verify/recovery；Plugin Preview→Review→Commit/inverse Undo；Domain HIGH `CREATE_BLOCK`/`MOVE_BLOCK`；ADR-0007 | Graph patch ledger、SemanticCommit、Undo | DOMAIN_EXTENSION | 原始事实丢失、部分 Commit | PARTIAL_FORMAL_UI_AUTOMATED：Preview→一个 HIGH Proposal，原材料 no-rewrite/no-delete；完整来源/最终结构指纹、执行前与补偿位置分离；每操作 GRAPH_WRITE step、顺序 verify、最终指纹、逆序 compensation、FAILED/replay 自动 PASS；独立 inverse Commit 和双向恢复已锁定，专用 Review/confirm/result/Undo UI 不落入通用单 Block 路径。同会话 Desktop custom UUID move/restore PASS；待完整正式 Desktop 链与 reload Rebind |
