...
| UX-R15 | Grill Me 固定问卷 | 每次都问同样字段 | 用户绕开 Project | session contract 已用 evidence-specific uncertainty priority 和 machine-owned stop condition 排除固定字段顺序；真实多轮 Provider/UI 仍需证明 | MITIGATED_CONTRACT_OPEN_RUNTIME |
