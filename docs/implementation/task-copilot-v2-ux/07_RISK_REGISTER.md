...
| UX-R15 | Grill Me 固定问卷 | 每次都问同样字段 | 用户绕开 Project | session contract + Service 两轮 route + 真实 DeepSeek 两轮已证明 boundary answer 后转向 outcome；Plugin 已消费机器 focus/readiness 而不渲染固定字段；独立 preview 已通过真实 Provider exact-once/root/zero-delete Gate，长期材料差异与 Desktop 仍需证明 | MITIGATED_PREVIEW_UI_AUTOMATED_OPEN_DESKTOP |
