...
- [ ] Restore/Migration 复用唯一安全链；Restore 已自动证明服务端有界目录、session token、
  ...
  当前产品 UI 的状态差异读回、反向 Restore、失败注入及 Migration 产品入口仍 OPEN；
- [ ] 高影响流程全部可恢复。
