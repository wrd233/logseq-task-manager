...
| Final Release | RELEASE_CANDIDATE_READY | 根级检查、145 条规则、恢复演练、安装态/authority、Skill、真实 Provider、Project/MiniProject create/Undo、当前自然 Block apply/Undo、zip 完整性与解压安装 PASS | `11e0131` r6 已直接安装 Launcher/Service/Plugin；包内 Runbook 与 HEAD 一致，稳定 Plugin 路径的 reload、quit/owned shutdown/reopen、Service 重连和 Doctor PASS 均有 CURRENT 证据 | r5 因包内 Runbook 漂移降为 `SUPERSEDED_PACKAGE`；r6 候选包可交付；完整长期 Goal 仍继续自然日用质量与真实发布后证据 |
