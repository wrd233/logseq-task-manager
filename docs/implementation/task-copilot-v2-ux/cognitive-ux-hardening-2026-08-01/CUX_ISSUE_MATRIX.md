...
| CUX-P0-02 | P0 | Durable Origin | UUID Page route reload 后空白，anchor 丢失 | 持久化+读回已有；reload 自动恢复/fallback 未实现 | reload/rename/move 后恢复 Page/Block；失败给一键 fallback | route resolver 测试 + Desktop reload/reopen | 待提交 |
...
| CUX-P1-02 | P1 | One-question Workspace | Grill 与创建器/关联/对象列表同屏 | Grill 已隔离底层；首屏仍先展示长报告 | 问题+输入置首；事实/推断/未知折叠 | DOM 顺序测试 + 三轮 Desktop | 待提交 |
| CUX-P1-03 | P1 | Progressive Disclosure | 日常层暴露 SQLite/Graph/Anchor/Association/Lifecycle 等 | 对象工作区仍全量暴露内部词 | 日常层用户语言；技术词进 details | 文本合同测试 + visual gate | 待提交 |
