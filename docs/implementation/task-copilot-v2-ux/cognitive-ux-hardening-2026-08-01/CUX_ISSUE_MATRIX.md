...
| CUX-P0-02 | P0 | Durable Origin | UUID Page route reload 后空白，anchor 丢失 | 已实现 + Desktop PASS：真实右键 Block 菜单 → 处理这条内容 → 返回原内容 → 路由为页面名+anchor（非 UUID）；hard reload 后页面正常加载、无空白/无 fallback 弹窗、BLOCK origin 持久化；同页时 resolver 会滚动恢复 anchor（11/11 单测） | reload/rename/move 后恢复 Page/Block；失败给一键 fallback | resolver 测试（11/11）+ Desktop reload 全链（2026-08-01） | 13f47ec + 本批 |
