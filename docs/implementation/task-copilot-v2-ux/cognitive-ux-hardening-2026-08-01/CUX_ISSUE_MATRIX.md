...
| CUX-P0-02 | P0 | Durable Origin | UUID Page route reload 后空白，anchor 丢失 | 已实现 + Desktop PASS（reload、quit/reopen、anchor 可见滚动、Page 改名+UUID 再生、同路由 boot 恢复）：稳定页面名路由、无空白/fallback、BLOCK origin 跨重载/重开持久化、解析器返回 RETURNED；anchor 滚动实测生效（滚动容器 0→411.84，目标块 910→498 进入视口；boot 恢复 top=498 inViewport）；修复路由事件无限循环与 FileStorage 早读时序；name-fallback 同页匹配 | reload/rename/move 后恢复 Page/Block；失败给一键 fallback | resolver 测试（12/12）+ Desktop 全链（2026-08-01） | 13f47ec + 本批 |
