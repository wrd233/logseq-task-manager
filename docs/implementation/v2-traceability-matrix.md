...
| E2E-11 | V2 §68 | Focus/期限/阻碍/Waiting 可解释排序 | Focus 非状态；Waiting 安静；无分数 | query now work | Now Work UI | VM + Desktop | E | DESKTOP_PARTIAL_PASS | 自动成功/失败合同保留。Desktop 已真实点击 Focus 加入、WAITING/BLOCKED 表单、Task 本地时间期限、可读 blocker 选择、阻碍对象唤醒、Task 筛选/按类型分组和 Primary Anchor 打开；CLI 读回对象 version 9 及 Condition/dueAt/blockerObjectId，reload 后 Focus/期限/状态保持，无分数。future-review Waiting 不再误报到期，已在 Focus 中的等待项不再显示重复加入操作。尚缺两项 Focus 手动上下排序、Project/Area 筛选、PAUSED/清除期限失败路径与键盘/主题 Gate |
