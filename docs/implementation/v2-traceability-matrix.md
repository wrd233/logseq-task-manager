...
| E2E-11 | V2 §68 | Focus/期限/阻碍/Waiting 可解释排序 | Focus 非状态；Waiting 安静；无分数 | query now work | Now Work UI | VM + Desktop | E | AUTOMATED_FOUNDATION | V2 纯投影与正式 UI 已证明 Focus 读取/加入/移出/手动排序、对象版本与完整顺序并发保护、Primary Anchor 打开入口、近期 actionable、到期 Waiting、普通 Waiting 静默、三区域空隐藏和无分数。Now Work 同页可正式设置 ACTIONABLE/WAITING/BLOCKED/PAUSED；WAITING 复查证据必填且到期立即进入复查区域。BLOCKED 可用可读 OPEN 对象选择器记录阻碍来源；不存在/关闭/自引用零写入，阻碍 Focus 的旧 actionable 或安静 Waiting 会按自然语言原因唤醒。类型筛选/分组不改变投影或正式状态，并在局部视图禁止误排完整 Focus。Task 明确期限以 schema v6 nullable due_at、版本化 Command 与自然语言原因进入时间排序，不生成分数；真实测试库 v3→v6 与独立 Service/CLI 已通过。reload/真实点击与 Desktop 待完成 |
