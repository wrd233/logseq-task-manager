...
| V2-DATA-001 | D-190..192；V2 §33-34 | 每 Graph 独立运行和恢复 | SQLite 唯一当前状态；schema 独立版本 | initialize/migrate/backup | SQLite + `.task-copilot/` | idempotency/locked/corrupt | A | AUTOMATED_FOUNDATION | Graph-bound schema v4；显式快照后 v1/v2/v3→v4 受控升级，v3 SemanticCommit step ledger 与 v4 Proposal/Group 表均有迁移记录；失败全量回滚/重试、事务幂等、corrupt/unknown、写锁零写入、Doctor、Backup 和 Restore 原语已自动测试。后续业务表只按 Slice 真实闭环增加 |
...
| V2-PROP-001 | D-094..122；V2 §23-28 | 可读、可拆分、可过期的建议 | Proposal 非事实；read/modify scope；group dependency | validate/revise/review/revalidate | 两文件、文本/语义 Diff | schema/stale/partial | C | AUTOMATED_FOUNDATION | V2 runtime Schema、两文件、scope/hash/risk/dependency、schema v4 持久化、submit/list/get/review 和 Review UI 已通过。C3 只重读全部 read scope 和 accepted modify target；Plugin 按 ID 取 Block/Page，Service 自取 SQLite OBJECT，缺证据/缺失/version/hash 变化持久化 `STALE`，成功不刷新审阅版本。UI 明示接受和检查通过仍未生效。尚缺 C4/C5 Commit/Undo 和 Desktop Gate |
| V2-COMMIT-001 | D-185..188；V2 §29 | 提交失败不假成功，可 Undo | version/hash；Partial Failure；Undo 不覆盖新编辑 | commit/undo/recover | Graph + SQLite step ledger | fault injection/restart | C | AUTOMATED_FOUNDATION | C4 单 Block 正式化已有 PENDING-first prepare、Graph before/after 重读、Application Object/Anchor/Audit/Receipt 物化、全 VERIFIED 完成门禁和 Proposal APPLIED。Domain 冲突后 VERIFIED Graph step 转 RECOVERY_REQUIRED，无后续编辑才逆写并标 COMPENSATED/FAILED；未知 finalize 只幂等重试，不盲目补偿。尚缺 C5 inverse Commit/Undo、restart recovery controller、Desktop/fault Gate，因此正式 UI Commit 仍锁定 |
