...
| V2-PROP-001 | D-094..122；V2 §23-28 | 可读、可拆分、可过期的建议 | Proposal 非事实；read/modify scope；group dependency | validate/revise/review | 两文件、文本/语义 Diff | schema/stale/partial | C | AUTOMATED_FOUNDATION | V2 runtime Schema、两文件确定性渲染、scope/hash/risk/dependency 校验、schema v4 Proposal/Group 原子持久化、submit/list/get/review 路由与乐观并发已通过。Review UI 同屏展示最终预览、文本/语义 Diff 和分组决定，HIGH 独立确认，并明示接受尚未生效。尚缺 C3 Stale/scope 正式提交重验、C4/C5 Commit/Undo 和 Desktop Gate |
