...
| V2-PROP-001 | D-094..122；V2 §23-28 | 可读、可拆分、可过期的建议 | Proposal 非事实；read/modify scope；group dependency | validate/revise/review | 两文件、文本/语义 Diff | schema/stale/partial | C | AUTOMATED_FOUNDATION | V2 runtime Schema、两文件确定性渲染、unknown shape、modify scope、Patch hash、risk 防降级、group/operation 唯一性与无环依赖已通过；语义组部分接受拒绝依赖链半截、不可独立组和未确认 HIGH。Service validate 路由零正式写入。尚缺 submit/read 持久化、最终 Review UI、Stale 重验与 Commit |
