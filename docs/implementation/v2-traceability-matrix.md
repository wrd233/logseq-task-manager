...
| E2E-10 | V2 §68 | Undo 正文和 Store，不覆盖后续编辑 | Undo 新 Commit | undo/recover | Graph + SQLite | fault + Desktop | C | DESKTOP_PARTIAL_PASS | 自动 inverse Commit、重启续跑、后续编辑零覆盖仍通过。Desktop 已真实 Commit 后从同一卡片 Undo：正文恢复，Object/Anchor 当前投影删除，正向 `UNDONE`、逆向 `COMPLETED`、Audit 保留，cold reload 状态一致；并修复插件自写 Graph 事件回声破坏严格 Undo。后续编辑拒绝与真实 Commit/Undo 进程故障尚缺。 |
