...
| E2E-12 | V2 §68 | 审阅中心原文优先、四处置 | Candidate/Proposal 分离 | disposition/review | two-tab Review | UI + Desktop | E | AUTOMATED_FOUNDATION | 待审阅 Proposal 卡片已实现上下文、最终预览、文本/语义 Diff、接受/拒绝/暂缓、最终 Commit 与 Undo；当前页显式 Candidate 已进入 Review Center 的“待整理”区域，保留手动有界扫描、Anchor 去重、单项同步和 stale 重读。Candidate 跨来源单一列表与拒绝/暂缓四处置、双标签最终布局及 Desktop Gate 尚未完成 |
