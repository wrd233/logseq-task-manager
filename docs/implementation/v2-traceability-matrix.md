...
| V2-ID-001 | D-030..041；V2 §21 | 移动/改名不丢身份 | object_id 独立；最多一 Primary Anchor | bind/observe/rebind | Graph UUID adapter | uniqueness/move/delete | A-B | REUSE_EVIDENCE | 初始绑定、观察、move/copy 与 rebind 后端合同已通过；rebind 强制确认、同类型与版本前置，旧 Anchor 保留 replaced，新 active Anchor/Object/Audit/Receipt 单事务且不改 object_id/Ownership；Plugin rebind 交互和 Desktop 仍待 Slice B |
...
| V2-COMMIT-001 | D-185..188；V2 §29 | 提交失败不假成功，可 Undo | version/hash；Partial Failure；Undo 不覆盖新编辑 | commit/undo/recover | Graph + SQLite step ledger | fault injection/restart | C | REUSE_EVIDENCE | V1 pending-first Saga/compensation/Undo 可复用；V2 SQLite 已实现 PENDING-first step ledger、受控跳步、全 VERIFIED 完成门禁、补偿门禁和重启 RECOVERY_REQUIRED 查询。Graph Adapter + Application Saga/Undo 仍待 Slice C |
...
| E2E-06 | V2 §68 | 删除 Primary Anchor | 对象保留、Anchor Conflict | consistency check/rebind | Graph adapter/review | fake + Desktop | B,E | REUSE_EVIDENCE | 已知 UUID 缺失/冲突观察保留 Object；同 UUID 可恢复 active；显式 rebind 后端可在确认后原子保留旧 replaced Anchor 并绑定新 UUID，类型/占用/stale/重试路径已测。Plugin rebind 交互与 Desktop 待完成 |
...
| E2E-15 | V2 §68 | Service 故障时正文可编辑 | 无 Service 不允许正式写入 | health/recover/check | Plugin restricted | process fault + Desktop | A | REUSE_EVIDENCE | Client 与 Plugin 诊断均自动证明 graphEditing=true/formalWrites=false；断线事件只进有界会话队列，READY 后重试，失败/溢出要求一致性检查且不写 FileStorage；reload 补漏与真实 Service stop/recover/Desktop 正文编辑待验证 |
