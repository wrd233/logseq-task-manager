...
Logseq Desktop 0.10.15 已验证 Task DONE、重复 DONE、移除 Marker 不重开、终态相反 Marker 冲突零写入，以及 CANCELED 缺少原因零写入。MiniProject 的 Marker 与对象列表三问、Agent 草拟和独立遗留承接均已补齐 Desktop 证据；详见 `docs/runtime/V2_MARKER_DESKTOP_REPORT.md`、`docs/runtime/V2_MINI_PROJECT_CLOSURE_DESKTOP_REPORT.md` 和 `docs/implementation/V2_MINI_PROJECT_CLOSURE_CONTRACT.md`。

Task 的产品内“记录取消原因后正式取消”与“显式重开”已完成自动与 Desktop 纵向闭环：对象页填写原因后只创建/修订同一机器 Proposal；Review 接受仍零正式写，最终精确确认后才经现有单步 Domain SemanticCommit 改变 SQLite Lifecycle。Domain 直接拒绝无原因取消/重开；原因保存在已应用 Proposal，正文、Anchor、Condition、Focus 与 Ownership 均不改变。Task 使用 MEDIUM 组，Project/MiniProject 保持 HIGH 组；未新增表、扫描器、恢复器或第二权威。Desktop 已通过空原因拒绝、取消、显式重开、reload、Service 中断后读回和 Lifecycle Undo；Logseq 原生 Undo 仍只能撤回正文 Marker，不能绕过 SQLite Lifecycle 或自动重开。证据详见 `docs/runtime/V2_REASONED_LIFECYCLE_AUTOMATION_REPORT.md` 与 `docs/runtime/V2_REASONED_LIFECYCLE_DESKTOP_REPORT.md`。
