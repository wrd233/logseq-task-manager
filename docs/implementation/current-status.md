...
- V1→V2 迁移已进入只读 Scan/Preview 自动基础：纯 Domain 覆盖全部旧 Phase/Condition/Signal，推进 Phase 只建议 OPEN 且保留信息损失，ACTIVE 不自动 Focus，Signal 只重算/转 issue；缺证据、冲突和非 V2 类型分别成为结构异常/待确认/普通内容。`tc migration scan` 显式读取最大 8 MiB Recovery Bundle，Service 验 schema/checksum/round-trip，并对 PENDING/RECOVERY_REQUIRED Commit 停止；报告不含正文，扫描前后 SQLite Doctor 完全一致。尚无 batch 表、Commit/Activate/Undo，`capabilities.migration=false`。
- SQLite schema v7 已只增加设计中明确要求的 `migration_runs` / `migration_batches` / `legacy_evidence`，用于 E2E-14 的 run 状态、≤50 项幂等批次/Undo 证据和旧标识映射依据，不存当前对象副本。`task-copilot-service migrate-schema` 必须显式给出 DB、Graph 和不存在的 Backup 路径，不启动 HTTP/Provider/descriptor；已用真实 v6 库证明 0600 快照、v6→v7、Service 重启与 CLI Doctor schema 7 PASS。当前尚未开放 run/batch 写入 API，`capabilities.migration=false`保持。
...
- 自动检查：2026-07-21 `./scripts/check.sh` PASS，305 tests、145 rules、0 skipped；typecheck、lint、build、package/bootstrap/dist、边界与恢复演练全过；npm audit 既有 2 high / 1 critical 未用破坏性 `audit fix --force`；
