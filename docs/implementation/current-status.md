...
V1 frozen / Slice A0 complete / Slice A1-A4 foundation in progress / Slice B0 complete / Slice B1-B2 foundation in progress / Slice B4 Anchor observation in progress
...
- Slice B4 rebind 后端安全闭环已补：精确 `REBIND_PRIMARY_ANCHOR` 确认由 Application 强制；Service 只接受旧 Anchor 引用与新 Block 证据并注入 Graph/actor/version/幂等边界；SQLite 单事务把旧 Anchor 保留为 `replaced`、建立唯一新 active Anchor、更新同一 object_id 的正文缓存/版本及 Audit/Receipt。未确认、类型变化、目标 UUID 已有当前或历史绑定、stale、重复请求及新 Anchor 插入中途故障均有零写入/幂等证据。Plugin 内 rebind 交互与 Desktop 仍待补。
...
- 自动检查：2026-07-20 `./scripts/check.sh` PASS，169 tests、145 rules、0 skipped；typecheck、lint、build、package/bootstrap/dist、边界与恢复演练全过；npm audit 同时报告现有依赖树 2 high / 1 critical，未运行破坏性 `audit fix --force`；
...
1. 继续 Slice B4：接入 Plugin 内显式 rebind 交互，并实现受控的新显式标识候选发现；不做频繁全 Graph 扫描；
