...
- 2026-07-20 Proposal Desktop Gate：authenticated Local Service 注入脱敏 user Proposal 后，真实 Review Center 完成上下文/Diff → MEDIUM 组接受但未生效 → 提交前重验 → 独立最终确认 → Graph+SQLite Commit → 同卡片 Undo → cold reload。Desktop 发现并修复成功后确认框残留、插件 Graph 写入自回声破坏 Undo、对象标题依赖回声补漏三条完整数据链问题。修复后 CREATE_OBJECT 强制最终 `payload.text`，精确一次性回声抑制不隐藏用户编辑；Commit 后 Graph/Task 文本一致且对象 version 2，Undo 后正文恢复、Object/Anchor 删除、正向 `UNDONE`、逆向 `COMPLETED`。高影响/拒绝/暂缓/stale/后续编辑与进程故障 Gate 仍待完成。

## 当前证据

- Git：`feature/task-copilot-mvp`；当前阶段包含 Service/CLI 基础与 SQLite 恢复加固；
- 自动检查：2026-07-20 `./scripts/check.sh` PASS，252 tests、145 rules、0 skipped；typecheck、lint、build、package/bootstrap/dist、边界与恢复演练全过；npm audit 既有 2 high / 1 critical 未用破坏性 `audit fix --force`；
