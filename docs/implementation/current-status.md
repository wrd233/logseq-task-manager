...
- SQLite schema v10 只新增设计既定的 `candidates` 审阅状态表：正文仍以 Logseq 为唯一权威，表内仅保存来源 UUID/版本、分类、理由、处置、当前 Proposal 与时间。来源 UUID + kind 保持一个稳定 Candidate；同版本重扫保留用户处置，来源变化才重开并把旧当前 Proposal 标为 `STALE`。v9→v10 必须先做精确只读快照，迁移、回滚和重试均有自动证据；没有新增扫描器、恢复器、写入路径或通用工作流状态机。Desktop 已真实通过离线当前页发现、原文优先、稍后/普通/同建议不再提示、CREATE Proposal READY/ACCEPTED 时 Object=0、最终 Commit RESOLVED、Undo reopen 和 Plugin reload。真实运行同时发现并修复 `id::` 属性回声提前物化：现用有界同正文窗口覆盖旧 traversal 与真实 echo，正文变化立即解除，timer 主动回收。UPDATE 已具备自动纵向闭环：用户选择一个已有 Block 对象并编辑完整最终正文，只生成单组 `REWRITE_BLOCK` Proposal；来源、目标 Anchor 和对象版本均重验，最终复用既有两步 SemanticCommit 同步同一 object_id，Undo 恢复旧正文并推进同一对象版本。没有新增表、Commit 类型或恢复器；Desktop 仍待验收。合同见 `docs/implementation/V2_CANDIDATE_UPDATE_CONTRACT.md`，运行证据见 `docs/runtime/V2_CANDIDATE_REVIEW_DESKTOP_REPORT.md`。
...
- 自动检查：2026-07-22 根级 `./scripts/check.sh` 为 369 tests、145 rules、0 failed/skipped；全部 typecheck/lint/test/build、Plugin/边界检查与恢复演练 PASS；npm audit 既有 2 high / 1 critical 未用破坏性 `audit fix --force`；
...
5. 在同轮 Desktop 验收 Review Center 当前页 Candidate 扫描、持久列表、三种非正式处置、Candidate→Proposal→Commit→Undo 与最终视觉；重点实测已完成自动基础的 `UPDATE` Candidate 目标选择、Diff、Commit、Undo、reload 与 stale 反馈，自动证据不替代该 Gate；
