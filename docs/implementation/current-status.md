...
- Primary Ownership HIGH Proposal 的 accepted-plan Validator 已建立：只接受唯一已接受 HIGH 组、单个 `CHANGE_OWNERSHIP`、零正文 Patch、带版本 child、新 Owner 和可选当前 Owner 前置；风险降级或复合操作在 Commit 规划前拒绝。正式 Domain Commit、Service/Plugin 审阅入口与 Undo 尚未接入，因此不宣称写闭环完成。
- Ownership Domain/Application/SQLite 原子变更内核已补：同时校验 child version 与精确当前 Owner，单事务更新 Object version、唯一 `primary_ownerships`、Audit 和幂等 Receipt；从 Unassigned 新增与已有 Owner 变更共用同一显式命令语义，stale 当前 Owner 零写入。当前没有直接 Service 写路由；只有后续 accepted HIGH Proposal SemanticCommit 可以调用它。
...
- 自动检查：2026-07-21 `./scripts/check.sh` PASS，340 tests、145 rules、0 skipped；typecheck、lint、build、package/bootstrap/dist、边界与恢复演练全过；npm audit 既有 2 high / 1 critical 未用破坏性 `audit fix --force`；
