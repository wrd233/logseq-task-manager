...
- Primary Ownership 已形成自动化纵向闭环：Domain/Application/SQLite 在同一事务校验 child version、new Owner version 与精确当前 Owner，并更新 Object version、唯一 `primary_ownerships`、Audit 和幂等 Receipt；accepted HIGH Proposal 专用 `/ownership/commit` 由服务端权威重读对象，固定确认后复用一个既有 DOMAIN_WRITE SemanticCommit step。Review Center 只为唯一 HIGH `CHANGE_OWNERSHIP` 显示专用确认，具备 busy 防重复、stale/错误反馈和成功后同一列表读回。未完成 Commit 会阻止 review/revalidate 改写恢复依据；故障注入证明 receipt 后中断可重启续完，prepare 后 new Owner 变化/删除会零写入并收口 FAILED/STALE，稳定但非法的 Ownership 类型矩阵会零写入并收口 Proposal/Commit FAILED；Commit FAILED 与 Proposal 终态之间再中断也可依据 ledger error code 幂等补齐。无直接 Ownership 写路由、平行恢复器或第二权威。Desktop 与 Ownership Undo 仍待完成，合同见 `docs/implementation/V2_OWNERSHIP_CHANGE_CONTRACT.md`。
...
- 自动检查：2026-07-21 `./scripts/check.sh` PASS，346 tests、145 rules、0 skipped；typecheck、lint、build、package/bootstrap/dist、边界与恢复演练全过；npm audit 既有 2 high / 1 critical 未用破坏性 `audit fix --force`；
