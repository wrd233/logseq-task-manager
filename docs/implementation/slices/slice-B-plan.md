...
| B4 | move / copy / delete / consistency（已知 Anchor 恢复、状态持久化、move/copy 与 rebind 后端合同完成） | UUID 复制不继承 ID；删除保留对象；rebind 必须独立确认 | READY 恢复及低频有界检查、原子观察、失败重试、move/copy 身份合同与显式 rebind Domain/Application/SQLite/Service/Client 已通过；Plugin rebind 交互待补 | 跨页移动、复制、删除、rebind |
...
- 尚未完成有限子树、Marker、Plugin rebind 交互，以及退出期间全新未绑定标识的受控候选发现；move/copy/rebind 仍需真实 Desktop 证据，因此完整 B1/B2/B4 Gate 与 E2E Desktop 仍未完成。
