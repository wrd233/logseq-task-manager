...
- schema v3 建立 SemanticCommit step ledger，schema v4 增加 Proposal/Group 审阅表，schema v5 解耦 immutable Audit 与当前 Object 投影以支持严格 Undo，schema v6 增加 nullable Task `due_at`；migration ledger 只追加，旧 schema 的显式升级在 DDL 前另建经校验的不覆盖快照。
