...
本 Gate 完成 V2 Area 的最小正式支持：创建、编辑、列表与版本冲突保护。Project 归属 Area 的 Domain/Proposal/Commit 能力已存在；后续同一真实 Area 已通过 Now Work 类型筛选与分组 Gate，键盘与主题也已补齐，证据见 `docs/runtime/V2_NOW_WORK_PROJECT_AREA_FILTER_DESKTOP_REPORT.md` 与 `docs/runtime/V2_VIEW_KEYBOARD_THEME_DESKTOP_REPORT.md`。
