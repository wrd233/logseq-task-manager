...
Undo is not a rewind. It inserts a new `UNDO_COMMIT`, validates that the target commit is still the latest semantic change and rechecks the current managed projection hash. Create compensation removes only the owned projection and current-state row; Rename compensation restores the previous title with a new monotonic version. Both verify the resulting Graph state, commit the compensation, and link the original row through `compensated_by`. Natural source content is outside the managed container and is never removed.
