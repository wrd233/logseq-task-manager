...
3. `engagement-reconciliation/0.1.1` constrains the Fake Agent to direct external prerequisites and explicit resolution facts; `0.1.0` remains immutable historical material.
...
6. One `CHANGE_ENGAGEMENT_FIELDS` effect writes Engagement and the human-readable WaitingCondition together in the stable managed state block. This one-block physical layout prevents any visible half-state and makes response-loss replay idempotent. Only a fresh readback can commit.
...
`reviewAt`, when supplied by an authorized semantic operation, adds `复查：...`. Phase 4 does not invent review dates or run a scheduler. Returning to ACTIONABLE removes the Waiting lines from that same owned state block in its single update; the natural record and unrelated blocks are untouched. The separate stable Waiting UUID remains reserved in the anchor contract for compatibility, but Phase 4 does not create a second mutable block.
