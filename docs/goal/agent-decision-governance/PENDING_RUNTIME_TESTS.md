# Consolidated Agent Governance Runtime Gate

The first bounded Desktop session ran on 2026-08-02. Preserve the results below and run only the remaining combinations in one later session (target: at most 30 minutes) against the same SQLite authority. Do not repeat destructive or authority-changing cases merely to make the matrix look full.

## Preconditions

- exact Goal build loaded from a stable path;
- Logseq 0.10.15, Local Service READY, Doctor recorded;
- EXPERIMENT default, Agent formal writes disabled;
- before counts captured for Object, Candidate, Proposal, Commit and governance rows;
- no credentials or private raw text in screenshots/logs.

## Representative scenarios

| # | Scenario | 2026-08-02 result | Remaining live evidence |
|---|---|---|---|
| 1 | Explicit Task Shadow | PASS: stable Block UUID, Chinese rule, real DeepSeek r2, reload persistence, zero formal count change | none for representative path |
| 2 | Weak signal | PASS: Review Signal only, no Candidate/Object, quiet UI, included in 60-day export | none for representative path |
| 3 | Multi-target | PASS: live exact-target retrieval drove bounded EXPANDED and invalid Provider output failed closed; no duplicate Task/Object | longer-run accuracy sampling only |
...
For every scenario record: exact build commit, Graph/page, runtime mode, before/after counts, user-visible result, service/Doctor result, screenshot IDs, whether proof is automated/Desktop/live-LLM, and unresolved limitations.
