...
| Node.js 20.20.x | **真实实测** | 全新 clean clone + `npm install` + user path（start/status/doctor/stop）实测通过；engines `>=20.19 <21` |
| Node.js 24 | **部分实测** | 开发环境运行全部 tests/soak；better-sqlite3 v12 前 native 崩溃已修；不作为正式支持声明 |
