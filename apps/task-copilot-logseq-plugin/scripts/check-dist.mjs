...
assert.match(html, /index\.js\?build=[0-9a-f]{12}/, "JavaScript asset must be keyed by the built commit");
assert.match(html, /index\.css\?build=[0-9a-f]{12}/, "CSS asset must be keyed by the built commit");
assert.doesNotMatch(html, /__TASK_COPILOT_BUILD__/, "build placeholder must not reach dist");
