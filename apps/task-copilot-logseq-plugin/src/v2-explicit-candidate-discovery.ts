...
export function renderV2ExplicitCandidateDiscoveryPanel(state: V2ExplicitCandidatePanelState, available: boolean, persistedCandidates: readonly V2Candidate[] = [], sourcePreviews: Readonly<Record<string, string>> = {}): string {
  ...
  const queue = visible.length ? `<section aria-label="待整理内容"><div class="eyebrow">待整理</div><h2>可能需要整理</h2>${actionable.length > visible.length ? `<p class="muted">当前显示前 ${visible.length} 项；处理后刷新即可继续查看其余 ${actionable.length - visible.length} 项。</p>` : ""}${visible.map((candidate) => `<article class="card compact"><div class="eyebrow">待确认的整理建议</div><h3>原始内容</h3><blockquote>${escapeHtml(sourcePreviews[candidate.candidateId] ?? "原文暂不可读；请先打开来源检查。")}</blockquote><p><strong>为什么出现在这里：</strong>${escapeHtml(candidate.reason)}</p><p><strong>建议：</strong>${escapeHtml(candidate.suggestion)}</p><p class="muted">来源位置：${escapeHtml(candidate.sourceAnchorId)}${candidate.deferredUntil ? ` · 复查 ${escapeHtml(new Date(candidate.deferredUntil).toLocaleString("zh-CN"))}` : ""}</p><div class="actions"><button type="button" data-action="v2-open-primary-anchor" data-value="${escapeHtml(candidate.sourceAnchorId)}">打开来源</button>${candidate.activeProposalId ? '<button type="button" class="primary" data-action="review-mode" data-value="proposals">审阅已有方案</button>' : `<button type="button" class="primary" data-action="v2-candidate-formalize" data-value="${escapeHtml(candidate.candidateId)}">整理为正式事项</button><button type="button" data-action="v2-candidate-update" data-value="${escapeHtml(candidate.candidateId)}">更新已有事项</button><button type="button" data-action="v2-candidate-later" data-value="${escapeHtml(`${candidate.candidateId}|${candidate.updatedAt}`)}">7 天后再看</button><button type="button" data-action="v2-candidate-dismiss" data-value="${escapeHtml(`${candidate.candidateId}|${candidate.updatedAt}`)}">保持普通内容</button><button type="button" data-action="v2-candidate-no-more" data-value="${escapeHtml(`${candidate.candidateId}|${candidate.updatedAt}`)}">以后不再提示</button>`}</div></article>`).join("")}</section>` : `<section class="card compact"><div class="eyebrow">待整理</div><h2>可能需要整理</h2><p>当前没有需要处理或到期复看的内容。</p></section>`;
