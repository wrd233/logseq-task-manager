...
export type ActionDialogKind =
  ...
  | "confirm-v2-creation-session-confirm"
...
function renderActionDialog(model: UiModel): string {
  ...
  const confirmations: Partial<Record<ActionDialogKind, [string, string, string]>> = {
    ...
    "confirm-v2-creation-session-confirm": ["确认正式创建", "我已检查完整 Draft Tree、来源与放置位置；这是最终确认，系统会接受同一 HIGH 方案并重新验证后创建正式对象，不需要再到审阅中心重复确认", "submit-v2-creation-session-confirm"],
