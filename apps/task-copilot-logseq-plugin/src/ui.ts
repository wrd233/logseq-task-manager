...
function renderReview(model: UiModel): string {
  ...
  const v2Cards = v2.map((record) => {
    ...
    <div class="notice">${canOwnershipUndo ? `Primary Ownership 已正式生效 · Commit ${escapeHtml(originalCommit.semanticCommitId)}；可恢复到审阅前主归属，且不会移动正文。` : ownershipUndoCommit?.status === "FAILED" ? "对象或 Primary Ownership 已有后续变化；Undo 已安全终止且没有覆盖当前状态。" : canUndo ? `已正式生效 · Commit ${escapeHtml(originalCommit.semanticCommitId)}；可撤销且不会覆盖后续编辑。` : originalCommit?.status === "UNDONE" ? "原 Commit 已撤销；Audit 与逆向 Commit 历史均保留。" : isProjectClosure && record.proposal.status === "APPLIED" ? `Project Closure 已正式生效 · Commit ${escapeHtml(originalCommit?.semanticCommitId ?? "")}；Project 已退出活跃视图，页面保留。` : isMiniProjectClosure && record.proposal.status === "APPLIED" ? `MiniProject 完成已正式生效 · Commit ${escapeHtml(originalCommit?.semanticCommitId ?? "")}；移除 Marker 不会自动重开，重开必须显式命令与理由。` : isOwnershipChange && record.proposal.status === "APPLIED" ? `Primary Ownership 已正式生效 · Commit ${escapeHtml(originalCommit?.semanticCommitId ?? "")}；位置、Anchor 与 Association 未改变。` : hasAcceptedGroup ? `已接受的语义组尚未正式生效；最终确认会在同一流程中重验并${isProjectClosure ? "原子记录 Closure 与完成状态" : isMiniProjectClosure ? "原子更新 MiniProject Lifecycle 与 Anchor 证据" : isOwnershipChange ? "原子改变唯一 Primary Ownership" : "写入并显示 Undo"}。` : "审阅决定只更新 Proposal；尚未修改正式正文或对象。"}</div>
  </article>`;
  }).join("");
...
function renderActionDialog(model: UiModel): string {
  ...
  const confirmations: Partial<Record<ActionDialogKind, [string, string, string]>> = {
    ...
    "confirm-v2-mini-project-closure": ["确认完成 MiniProject", "我已审阅 DONE 关闭请求；系统将重验 Block、Anchor 和对象版本，然后原子完成 MiniProject", "submit-v2-mini-project-closure"],
