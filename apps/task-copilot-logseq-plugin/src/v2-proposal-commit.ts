...
export async function commitV2Formalization(
  ...
  if (prepared.status === "RECOVERY_REQUIRED") {
    ...
    if (prepared.plan.patch.beforeHash !== prepared.plan.patch.afterHash) await host.updateBlock(prepared.plan.patch.blockUuid, prepared.plan.patch.beforeText);
  ...
  if (before.hash === prepared.plan.patch.beforeHash && prepared.plan.patch.beforeHash !== prepared.plan.patch.afterHash) await host.updateBlock(prepared.plan.patch.blockUuid, prepared.plan.patch.afterText);
...
export async function undoV2Formalization(
  ...
  if (prepared.status === "RECOVERY_REQUIRED") {
    ...
    if (prepared.patch.beforeHash !== prepared.patch.afterHash) await host.updateBlock(prepared.patch.blockUuid, prepared.patch.afterText);
