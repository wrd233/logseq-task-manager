...
export function renderRuntimeDiagnostics(snapshot: RuntimeDiagnosticsSnapshot): string {
  ...
  return `<section class="app-shell diagnostics-shell" data-task-copilot-ui="${UI_NAMESPACE}">
    <header class="topbar"><div><div class="eyebrow">Runtime Diagnostics</div><h1>Task Copilot</h1></div><div class="top-actions"><button data-action="copy-diagnostics" class="primary">Copy diagnostics</button><button data-action="close" class="quiet">关闭</button></div></header>
    <main class="workspace diagnostics-workspace">
      ...
      <div class="diagnostic-grid"><section><h2>Runtime 状态</h2><p>${escapeHtml(snapshot.runtime_status)}</p></section><section><h2>Store 状态</h2><p>${escapeHtml(snapshot.store_status)} · schema ${escapeHtml(snapshot.store_schema)}</p></section><section><h2>当前 Graph</h2><p>${escapeHtml(snapshot.current_graph)}</p></section><section><h2>版本</h2><p>Plugin ${escapeHtml(snapshot.plugin_version)} · Logseq ${escapeHtml(snapshot.logseq_version)}</p></section></div>
