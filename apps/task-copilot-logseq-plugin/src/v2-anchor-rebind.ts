...
export type V2RebindPanelState =
  | { status: "idle" }
  | { status: "capturing"; candidateObjectId?: string }
  | { status: "loading"; candidateObjectId?: string }
  | { status: "ready"; preview: V2RebindPreview; serviceGeneration: number; busy?: boolean }
  | { status: "success"; message: string; candidateObjectId: string }
  | { status: "error"; message: string; candidateObjectId?: string };
...
function targetFromBlock(value: unknown): V2RebindPreview["target"] {
  ...
  if (!block) throw new Error("当前没有选中可读取的 Logseq 正文；没有执行重新连接。");
  if (block.parsed.kind !== "OBJECT") {
    throw new Error(block.parsed.kind === "INVALID"
      ? "当前正文的类型标识存在冲突或缺少标题；没有执行重新连接。"
      : "当前正文不是 [任务]、[MiniProject]、[决策] 或 [成果]；没有执行重新连接。");
  }
  return {
...
export async function prepareV2PrimaryAnchorRebind(
  ...
  if (page.anchors.some((anchor) => anchor.externalId === target.externalId && anchor.status !== "replaced")) {
    throw new Error("当前正文已经连接到另一个正式事项；没有进入确认，也没有执行写入。请取消本次选择，再从“开始重新连接”进入受控窗口后新建或选择替换正文。");
  }
  ...
  const candidates = page.anchors.flatMap((anchor) => {
    ...
    const candidateEligible = candidateObjectId
      ? anchor.objectId === candidateObjectId
      : anchor.status === "missing" || anchor.status === "conflict";
    if (!candidateEligible || !object || object.objectType !== target.objectType || anchor.externalId === target.externalId || anchor.status === "replaced") return [];
    return [{ anchor: anchor as V2RebindCandidate["anchor"], object }];
  });
  if (candidates.length === 0) {
    throw new Error(candidateObjectId
      ? "当前有界范围内没有找到这项正式事项的可用连接；没有执行写入。请返回系统状态重新检查。"
      : "当前没有与所选正文同类型、且确实需要修复连接的正式事项；没有执行写入。");
  }
...
export async function submitV2PrimaryAnchorRebind(
  ...
  if (
    ...
    throw new Error("当前正文已在预览后变化；请重新打开预览，旧预览没有提交。");
  }
  ...
  if (
    ...
    throw new Error("当前正文的持久身份建立后发生变化；没有提交重新连接。");
  }
...
function objectTypeLabel(value: V2ManagedObject["objectType"]): string {
  return {
    AREA: "领域",
    PROJECT: "项目",
    TASK: "任务",
...
export function renderV2PrimaryAnchorRebindPanel(state: V2RebindPanelState, available: boolean): string {
  if (!available) return "";
  if (state.status === "idle") return `<section class="diagnostic-notice"><h2>重新连接正文</h2><p>只读取当前选中的正文与一页已知连接；不会扫描整个知识库。</p><button type="button" data-action="v2-rebind-open">预览重新连接当前正文</button></section>`;
  if (state.status === "capturing") return `<section class="diagnostic-notice" aria-label="选择替换正文"><h2>选择替换正文</h2>
    <p>本次受控窗口会暂缓自动创建新的正式事项，最多持续 5 分钟。请回到 Logseq，新建或选中一条带有明确类型标识的替换正文，再回来预览影响。</p>
  ...
  if (state.status === "loading") return `<section class="diagnostic-notice" aria-busy="true"><h2>重新连接正文</h2><p>正在读取当前正文与已知连接…</p></section>`;
  ...
  if (state.status === "success") return `<section class="diagnostic-notice"><h2>正文已重新连接</h2>
    ...
    <p class="muted">如果选错了正文，不要删除正式事项或旧连接历史：先选中正确正文，再次进入受控重新连接。只有需要回退整个正式状态时，才使用备份与恢复。</p>
    <div class="actions"><button type="button" data-action="v2-rebind-capture" data-value="${escapeHtml(state.candidateObjectId)}">重新选择正文</button><button type="button" data-action="backup-restore-open">查看完整恢复选项</button><button type="button" data-action="v2-rebind-cancel">关闭</button></div>
  </section>`;
  const { preview } = state;
  ...
  return `<section class="diagnostic-notice" aria-label="重新连接正式事项正文"><h2>预览重新连接正文</h2>
    <p><strong>当前选中的新正文：</strong>${escapeHtml(objectTypeLabel(preview.target.objectType))} · ${escapeHtml(preview.target.text)}</p>
    <label>选择要重新连接的正式事项<select data-field="v2RebindCandidateToken"><option value="">请选择</option>${preview.candidates.map(({ anchor, object }, index) => `<option value="candidate:${index}">${escapeHtml(object.text)} · ${escapeHtml(objectTypeLabel(object.objectType))} · ${escapeHtml(statusLabel(anchor.status))}</option>`).join("")}</select></label>
    ${preview.moreAnchorsDeferred ? "<p class=\"muted\">候选超过当前有界页；未扫描后续页或整个知识库。</p>" : ""}
    <p>确认后，正式事项与主归属不变；旧连接保留在历史中，当前选中的正文成为唯一主正文。</p>
    <label class="confirm-line"><input type="checkbox" data-field="v2RebindConfirmed">我确认把这个正式事项重新连接到当前选中的正文</label>
    <div class="actions"><button type="button" class="danger" data-action="v2-rebind-submit"${state.busy ? " disabled aria-busy=\"true\"" : ""}>${state.busy ? "提交中…" : "确认重新连接"}</button>${state.busy ? "<span class=\"muted\">正式请求已提交，请等待明确结果。</span>" : '<button type="button" data-action="v2-rebind-cancel">取消</button>'}</div>
  </section>`;
}
...
export function renderV2AnchorIssueStatus(
  ...
  return `<section class="anchor-issue-status" aria-label="正文连接待处理">
    ...
    ${available && actionable
      ...
      : `<p class="muted">当前正式修改已暂停；Task Copilot 服务恢复后再处理正文连接。</p>`}
  </section>`;
}
