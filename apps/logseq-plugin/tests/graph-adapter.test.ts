...
const effect: GraphEffect = { type: "UPSERT_MANAGED_PROJECTION", graphId: "graph-01", sourceBlockUuid: "source-01", projection: { ...core, projectionHash: stableHash(core) } };
...
test("remove refuses an edited or expanded managed container", async () => {
  ...
  await assert.rejects(adapter.applyGraphEffect({ type: "REMOVE_MANAGED_PROJECTION", graphId: "graph-01", sourceBlockUuid: "source-01", containerUuid: core.containerUuid, expectedProjectionHash: effect.projection.projectionHash }), /GRAPH_REMOVE_PRECONDITION_FAILED/u);
