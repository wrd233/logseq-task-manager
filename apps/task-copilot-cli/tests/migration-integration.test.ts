...
test("CLI completes one copied-data migration through the real Local Service", async (t) => {
  ...
  const service = await startLocalService({ databasePath: join(root, "task-copilot.db"), backupRoot: join(root, "backups"), graphId: "graph-cli-migration", token: "cli-migration-integration-token-24-chars" });
  t.after(async () => { await service.close(); await rm(root, { recursive: true, force: true }); });
  const descriptor: ServiceDescriptor = { protocolVersion: 1, url: service.url, token: service.token, pid: process.pid, createdAt: "2026-07-21T08:00:00.000Z" };
