  ...
  return {
    ...
    domain: {
      ...
      content: [
        ...
        "TASK 是一个近期、可一次判断完成的承诺，可以包含为同一结果服务的相邻动作；不要只因为一句话含“并且”或先后两个动作就升级为 MINI_PROJECT。MINI_PROJECT 必须有多个可独立跟踪的步骤，并以一个有限结果收口。若同时满足：(1) 两个以上并列处理对象；(2) 至少两个可分别验收的处理阶段；(3) 还要形成独立交付物，则必须判为 MINI_PROJECT，不得降为 TASK。DECISION 是已确认且持续影响后续行为的选择；OUTPUT 是已经产出的可复用成果。",
        "边界对比例：‘周四前核对一次发布范围并通知当班人员’是 TASK；‘本月内梳理四组权限、逐组验证并整理移交清单’是 MINI_PROJECT。不要复制示例正文，只应用边界。",
        "OUTPUT 必须有可追溯的产出者或来源；上下文未提供时只能放入 unresolvedQuestions，不能发明 produced_by。",
        "已知 Project 上出现明确、有限、可闭合的具体缺口时，可以为关闭该缺口提议一个局部 TASK，并把已知 Project 只放在 read scope；普通进展或背景状态仍返回 NO_PROPOSAL，绝不自动确认 Ownership。",
        "runtime 提供 currentIntent 时只修订同一意图并保留其中事实，不创建第二意图；runtime 提供 confirmed=false 的 ownershipCandidate 时只保留候选与未确认语义，不输出 CHANGE_OWNERSHIP。",
        ...
        "如果 evidence 不足，输出且只输出 {\"decision\":\"NO_PROPOSAL\",\"reason\":\"简洁理由\"}。",
      ].join("\n"),
    },
    skill: {
