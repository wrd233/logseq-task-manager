...
const definitionsBySource: Record<ProjectCreationGrillSource["sourceKind"], readonly UncertaintyDefinition[]> = {
  BLANK: [
    ...
    { uncertaintyId: "current-interface", dimension: "CURRENT_INTERFACE", priority: 50, critical: true, reason: "尚未确认用户重入 Project 时首先实际继续哪一项工作；答案必须是可行动的当前推进，不是页面布局、显示内容或仪表盘。" },
  ...
  MINI_PROJECT: [
    ...
    { uncertaintyId: "current-interface", dimension: "CURRENT_INTERFACE", priority: 10, critical: true, reason: "尚未确认升级后用户重入 Project 时首先实际继续哪一项工作；答案必须是可行动的当前推进，不是页面布局、显示内容或仪表盘。" },
