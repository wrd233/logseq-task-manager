...
- **Natural-language USER authorization is compiled, never impersonated.** A mature `DecisionPackage` + unambiguous acceptance becomes an immutable `UserDecision`, then a precise `USER`-actor Formal Commit with projection obligation. Quoted, historical, conditional, ambiguous, or stale utterances never mutate Formal State.
