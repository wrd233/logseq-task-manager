...
const cli = new URL("../apps/task-copilot-cli/dist/main.js", import.meta.url);
const source = await readFile(cli, "utf8");
if (!source.startsWith("#!/usr/bin/env node\n") || source.slice(2).includes("#!/usr/bin/env node")) throw new Error("CLI_SHEBANG_INVALID");
const smoke = spawnSync(process.execPath, [cli.pathname, "--help"], { encoding: "utf8" });
...
const missing = spawnSync(process.execPath, [cli.pathname, "status", "--json"], { encoding: "utf8", env: { ...process.env, TASK_COPILOT_DESCRIPTOR: missingPath } });
