...
for (const [entry, output] of [
  ...
  await build({ entryPoints: [new URL(`../${entry}`, import.meta.url).pathname], bundle: true, platform: "node", format: "esm", target: "node20", outfile: new URL(`../${output}`, import.meta.url).pathname, packages: "external", banner: output.includes("task-copilot-cli") ? { js: "#!/usr/bin/env node" } : undefined });
}
